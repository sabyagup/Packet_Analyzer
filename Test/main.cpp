...
// add the packet paylod into a has table, entry is set according to a pair {ip,port}
    ...
    { 
		...
		if ( match(ntohs(tcp->th_sport),*prot))
		{
			...
			if ( !flow.count(make_pair(ip_addr,ntohs(tcp->th_dport))))
				flow.insert( make_pair(make_pair(ip_addr,ntohs(tcp->th_dport)),string()));
			it = flow.find(make_pair(ip_addr,ntohs(tcp->th_dport)));
			s.append(it->second);
